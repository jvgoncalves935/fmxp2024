...
public class MenuPausaManager : MonoBehaviour
{
    ...
    private void CarregarStrings() {
        stringsCutscene = LocalizationSystem.GetDicionarioStringsCena("MenuPausa");
    }

    private void AplicarStrings() {
        botaoContinuar.transform.GetChild(0).GetComponent<TMP_Text>().text = stringsCutscene["MENU_PAUSA_CONTINUAR"];
        botaoMenuPrincipal.transform.GetChild(0).GetComponent<TMP_Text>().text = stringsCutscene["MENU_PAUSA_VOLTAR_MENU"];
        botaoSair.transform.GetChild(0).GetComponent<TMP_Text>().text = stringsCutscene["MENU_PAUSA_SAIR"];
    }
